...
var _mz_saUrlPrefix = ''; //POSSIBLE OPTIONS:
//Win
//Windows
//Mac
//iPad
...
function OS(os) {
  ...
  if (os === "smartactivatorweb" && typeof SMARTACTIVATORWEB !== "undefined" && SMARTACTIVATORWEB === true) {
    return true;
  } else if (navigator.appVersion.toLowerCase().indexOf(os) != -1) return true;else return false;
}
...
function stringFromTimeInMilliseconds(time, fullYear) {
  ...
  if (!fullYear) year = year.substr(year.length - 2, 2);
  var hour = n(d.getHours());
  var min = n(d.getMinutes());
  var sec = n(d.getSeconds()); // MM-dd-yy HH:mm:ss
  ...
  for (var i = 0; i < elem.length; i++) {
    if (elem[i].getAttribute('id') != 'EmailDomain' && elem[i].getAttribute('id') != 'Domain' && elem[i].getAttribute('id').substring(0, 3) != "mz_") {
      if (elem[i].type == "text" || elem[i].type == "textarea" || elem[i].type == "number" || elem[i].type == "tel" || elem[i].type == "email") {
        if (elem[i].value.trim() != '') result += elem[i].getAttribute('id') + "=" + encodeForiOS(elem[i].value) + ";";
      } else if (elem[i].type == "select-one") {
        if (elem[i].options.length > 0 && elem[i].options[elem[i].selectedIndex].value.trim() != '') result += elem[i].getAttribute('id') + "=" + encodeForiOS(elem[i].options[elem[i].selectedIndex].value) + ";";
...
function GetDelimiter() {
  if (OS("SMARTACTIVATORWEB")) return ";";else if (OS("Mac")) {
    return "&";
  } else {
    return ";";
  }
